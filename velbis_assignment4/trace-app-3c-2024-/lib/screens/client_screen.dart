      ...
      drawer: Drawer(
        child: Column(
          children: [
            ...
            TextButton(
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(builder: (_) => ProfileClientScreen(userId: userId)));
              },
              child: Text("Profile"),
            ),
            TextButton(
              onPressed: () async {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => HistoryScreen(visitHistory: visitHistory)),
                );
              },
...
class HistoryScreen extends StatefulWidget {
  ...
  HistoryScreen({Key? key, required this.visitHistory}) : super(key: key);
...
Future<String> fetchBusinessName(String establishmentId) async {
  ...
  Widget build(BuildContext context) {
    ...
    List<Map<String, dynamic>> filteredHistory = widget.visitHistory.where((visit) {
      ...
      return visitDate.year == setDate.year &&
          visitDate.month == setDate.month &&
          visitDate.day == setDate.day;
    }).toList();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ListTile(
            title: Text('Selected Date: ${DateFormat('EEEE, MMM d, y').format(setDate)}'),
            trailing: IconButton(
